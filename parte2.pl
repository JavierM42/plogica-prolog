...
%chequearColumnas(+Tablero)  para cada columna de Tablero, chequea que las restricciones sean correctas.
chequearColumnas([]).
chequearColumnas([C|R]) :- chequearColumna(C), chequearColumnas(R).

%chequearColumna(+Columna) chequea que Columna cumpla con las sumas correspondientes.
chequearColumna([]).
...
%chequearBloqueColumna(+Suma, +Acc, +Columna) chequea que los elementos del bloque sumen Suma, y sean diferentes entre si.
